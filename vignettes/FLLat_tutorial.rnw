...
\texttt{FLLat.FDR}.  The \texttt{FLLat} function fits the Fused Lasso Latent Feature model for
...
every location with an absolute fitted value larger than $T$ is
declared a significant aberration.  Based on these declared
...
the probe locations for every sample.  We can then estimate the FDR by
\begin{equation}
\label{eq:permfdr}
\widehat{\mbox{FDR}(T)} =
...
upper bound for the FDR, or it can be set to something less than 1 if
...
controls the FDR at 0.05 and plots the FDRs against the threshold values.
